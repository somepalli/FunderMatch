...
class DecidedLoanCase(BaseModel):
    ...
    case_id: str = Field(pattern=r"^SYN-[0-9]{3}$")
    ...
    def validate_internal_consistency(self) -> DecidedLoanCase:
        ...
        if any(
            not metric.citation.document_id.startswith("synthetic-") for metric in self.evidence
        ):
            raise ValueError("Phase 1 corpus may contain synthetic document IDs only")
        return self
